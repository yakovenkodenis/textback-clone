...
//     if (typeCheck(props, propName, componentName) === true) {
//       return notAllowed
//              .filter((notAllowedPropName) => props[notAllowedPropName] !== undefined ).length > 0 ?
//              new Error(`${componentName}: if ${propName} is set ${notAllowed.join(',')} aren't allowed.`) :
//              true;
//     }
//   };
// };
...
export default class Thumbnail extends Component {
  ...
  showWhenReady(element) {
    ReactDOM.findDOMNode(element).addEventListener('load', (e) => {
  ...
  renderThumbnail({ children, page, scale, interactive, pageWidth, pageHeight, keepAspectRatio, showWhileLoading, state}) {
    ...
    const sourceProps = page !== undefined ?
                        { src: page } :
                        { srcDoc: ReactDOMServer.renderToStaticMarkup(children) };
    ...
    return (
        <div style={ containerStyle }>
            <iframe
                ...
                ref={this.showWhenReady.bind(this)} 
                style={ iFrameStyle }
                { ...iFrameProps }
            />
        </div>
